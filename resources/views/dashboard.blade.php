<x-layouts::app :title="__('Dashboard')"> <div class="flex h-full w-full flex-1 flex-col gap-8">
    ...
    <div class="overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-700 dark:bg-zinc-900">
        ...
        <form
            ...
            class="grid gap-4 p-6 md:grid-cols-6"
        >
            <flux:input
                name="search"
                label="Search"
            ...
            <flux:select name="location" label="Location">
            ...
            <flux:select name="machine_type" label="Machine Type">
                <option value="" @selected($machineType === '')>
                    All
                </option>

                @foreach ($machineTypes as $machineTypeOption)
                    <option
                        value="{{ $machineTypeOption }}"
                        @selected($machineType === $machineTypeOption)
                    >
                        {{ $machineTypeOption }}
                    </option>
                @endforeach
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center gap-2">
                                        ...
                                        @if ($search || $location || $machineType || $status || $maintenance)
                                            <flux:button
                                                size="sm"
                                                variant="ghost"
                                                href="{{ route('dashboard') }}"
